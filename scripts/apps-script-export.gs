/**
 * E27-T01: eski Sheets byudjetini JSON'ga eksport qilish (v1 formati).
 *
 * Qo'llanma: docs/MIGRATSIYA.md. Qisqacha:
 *   1. Jadval → Kengaytmalar → Apps Script;
 *   2. shu faylni `Eksport.gs` nomi bilan qo'shing (mavjud `Code.gs`
 *      yonida — yordamchi funksiyalar o'shanda);
 *   3. muharrirda `eksportniSaqla` funksiyasini ishga tushiring;
 *   4. Drive'dagi `my-wallet-export.json` ni yuklab oling.
 *
 * Ma'lumot Google akkauntingizdan chiqmaydi: web-app ham, ommaviy havola
 * ham kerak emas (fayl faqat o'z Drive'ingizda yaratiladi).
 */
function eksportniSaqla() {
  var json = JSON.stringify(eksportJson_(), null, 2);
  var fayl = DriveApp.createFile('my-wallet-export.json', json, MimeType.PLAIN_TEXT);
  var xabar = "Fayl tayyor: " + fayl.getUrl();
  try {
    SpreadsheetApp.getUi().alert(xabar);
  } catch (err) {
    // Muharrirdan ishga tushirilganda UI bo'lmaydi — jurnalga yoziladi.
    Logger.log(xabar);
  }
  return fayl.getUrl();
}
...
/**
 * Sheets → JSON (v1): sozlamalar, daromad, xarajat, "O'zim uchun"
 * sarflari, qarz, maqsad va Sheets o'zi hisoblagan oylik yakunlar
 * (`expectedMonths` — import to'g'riligini tekshirish uchun etalon).
 *
 * Summalar so'mda (butun son) — yangi tizimda tiyinga o'tkaziladi.
 */
