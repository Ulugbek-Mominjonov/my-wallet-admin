...
| `planned_items` | `kind`, `name`, `category_id`, `account_id`, `planned_amount bigint null`, `due_date`, `budget_month date`, `auto_pay`, `debt_id`, `recurring_rule_id`, `system_code`, `paid_amount bigint` (trigger), `settled_at`, `skipped_at`, `note` | unique(`recurring_rule_id`,`budget_month`); unique(household, budget_month, system_code) |
| `transactions` | `kind` (`income`/`expense`/`transfer`), `account_id`, `to_account_id`, `amount bigint >0`, `to_amount`, `currency`, `amount_base`, `fx_rate`, `category_id`, `payee`, `occurred_on date`, `budget_month date`, `budget_month_source` (`auto`/`manual`), `planned_item_id`, `debt_id`, `note`, `source` (`manual`/`quick_action`/`auto_pay`/`import`/`telegram`) | kind'ga mos CHECK'lar (transfer ⇒ to_account, category yo'q) |
...
| `tx_derive` (BEFORE) | `transactions` | `budget_month` (source=auto: income → oy+shift; boshqasi → sana oyi; reja bog'langan → reja oyi), `currency`, `amount_base`, kind CHECK'lari | BR-040..046, BR-191 |
| `tx_after` (AFTER) | `transactions` | bog'langan reja(lar) `paid_amount` qayta hisobi; daromad o'zgarsa ajratma rejasini qayta hisoblash; limit chegarasi → outbox | BR-071, BR-060, BR-133 |
